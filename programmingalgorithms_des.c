...
void programmingalrorithms_des(uint8_t * text, uint8_t* out) {
    uint8_t customKey[8] = { 0x01,0x23,0x45,0x67,0x89,0xAB,0xCD,0xEF };
    uint8_t schedule[16][6];

    // encrypt
    KeySchedule(customKey, schedule, ENCRYPT);
    Crypt(text, out, schedule);
    if (DEBUG)  { PrintText(out); }

    // decrypt
    KeySchedule(customKey, schedule, DECRYPT);
    Crypt(out, text, schedule);
    if (DEBUG) { PrintText(text); }
}